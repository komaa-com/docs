...
|---|---|---|
| `PORT` | `8080` | Port the media WebSocket listens on. Register `/msteams/calling` as its path - there is no path variable; the bridge reads the **last** path segment of the upgrade URL as the `callId` and accepts whatever base you put in front of it. `/healthz` and `/metrics` stay at the root. |
