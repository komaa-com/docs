...
| **Path** | `/msteams/calling/{callId}` on the StandIn-maintained bridges (Hermes, OpenClaw); `/voice/msteams/stream/{callId}` on the ElevenLabs, LiveKit, OpenAI, Deepgram and Cartesia bridges. StandIn derives the first from a bare host, and respects any full URL you paste, so either works. |
