...
| **Path** | `/msteams/calling/{callId}` on Hermes, OpenClaw and LiveKit - LiveKit now ANCHORS on that base and rejects anything outside it. ElevenLabs takes the LAST path segment as the callId, so it accepts whatever path you register; `/voice/msteams/stream/{callId}` is the conventional one. StandIn derives the first from a bare host, and respects any full URL you paste, so either works. |
