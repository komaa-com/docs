...
keywords: ["Teams voice architecture","HMAC WebSocket bridge","StandIn media bridge","Teams calling media","Microsoft Graph permissions","avatar rendering"]
---
...
flowchart LR
    ...
    P["Teams Bridge Plugin<br/>(WebSocket server)"]
...
The **plugin is the WebSocket server** (default calling port `:9442` on every runtime: OpenClaw,
Hermes, and the LiveKit and ElevenLabs bridges); the **hosted StandIn bridge is the client** that
connects to it. You register your plugin's WebSocket URL and shared secret in your StandIn dashboard
- expose it so the hosted bridge can reach it (a public URL, or a tunnel such as Tailscale).

Only OpenClaw and Hermes also serve a **chat lane** (`/api/messages`). The LiveKit and ElevenLabs
bridges are voice only: one calling WebSocket, plus `/healthz` and `/metrics`, and no messages
...
| **Teams Bridge Plugin** (WS server) | hosts the bridge · realtime / streaming dialogue · vision ring + budget · emits `expression` / `speech.marks` / `display.image` cues |
...
| Aspect | Value |
|---|---|
| **Handshake** | `HMAC-SHA256(secret, "{timestampMs}.{callId}")`, lowercase hex, sent as `X-StandIn-Timestamp` / `-Signature` (legacy `X-OpenClawTeamsBridge-*` still accepted) headers on the WS upgrade. ±60 s window; each `(callId, ts, sig)` is single-use (replay-guarded). |
| **Path** | `/msteams/calling` on all four runtimes; StandIn appends `/{callId}` per call. Hermes and LiveKit **anchor** on that base and reject anything outside it, so the segment is required. OpenClaw serves it bare (a missing `callId` is a `400`). ElevenLabs takes the LAST path segment as the callId and so accepts whatever base you register - use `/msteams/calling` there too. StandIn derives the path from a bare host, and respects any full URL you paste. |
...
<Warning>
  The plugin's `secret` **must byte-match** the value registered for your bot in StandIn, or the
