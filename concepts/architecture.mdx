---
title: "Architecture"
description: "How the StandIn media worker, the HMAC WebSocket bridge, the plugin, and your agent fit together."
---

Teams real-time call media (`Microsoft.Skype.Bots.Media`) is **Windows/.NET-only**, so rendering and
RTP are handled by a separate **StandIn media worker**. The plugin is the cross-platform **brain**: it
...
| **StandIn worker** (Windows / .NET) | renders the avatar tile · samples inbound A/V, forwards DTMF · recording-status compliance gate · places outbound calls |
| **Microsoft Teams** | the live call / meeting and its media plane |

<Note>
  The StandIn worker is a separate component. Run **one worker identity per gateway**; its
  multi-identity config lets a single host serve multiple gateways.
</Note>
...
<Warning>
  The plugin's `sharedSecret` **must byte-match** the StandIn worker's shared secret, or the HMAC
  handshake fails and no call connects.
</Warning>
