...
keywords: ["Teams bot features","screen and camera vision","group call etiquette","outbound call back","meeting recap","DLP redaction","bilingual","DTMF"]
---
...
- **End-of-meeting recap** (opt-in) + on-demand **`post_meeting_minutes`**, posted to the Teams chat
  with per-speaker attribution, the minutes attached as a Word **`.docx`** file card. On Hermes the
  card rides the Bot Framework attachment contract (bot credentials only, no SharePoint); SharePoint
  upload (`Sites.ReadWrite.All`) remains an optional path where configured.
