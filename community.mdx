...
calls), capped at 5 minutes of call time per day, one per account. Choose it on the
[dashboard](https://standin.komaa.com/dashboard) ("Free - capped") or at the pairing approve step.
...
<Note>
Running **Hermes**? Start the voice endpoint with `hermes teams-voice serve --handler realtime`. It
listens on port **8443**, so the URL you connect in the sandbox is
`wss://<your-public-host>:8443/voice/msteams/stream`, with the `shared_secret` from your Hermes
config (`hermes teams-voice status` shows both). OpenClaw (`openclaw gateway run`) listens on
**9442** locally; either way, connect the **public** `wss://` URL with its port, not the local bind.
</Note>
