...
turn-taking in one model) onto a Microsoft Teams call. It hosts the HMAC WebSocket that StandIn
connects to, then relays audio to and from an OpenAI Realtime session.

<Note>
  This is a **standalone Node service**, not a framework plugin. There is
...
The bridge binds its media WebSocket (default `ws://<host>:9442/voice/msteams/stream`; StandIn
...
Your **Agent calling URL** is then:
...
1. Set the identity's **Agent calling URL** to where the bridge listens.
