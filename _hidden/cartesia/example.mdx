...
The bridge listens on port `9442`. Expose it with a tunnel (Tailscale Funnel, Cloudflare Tunnel,
...
Set your identity's **Agent calling URL** to that `wss://` URL in the
...
There is no vision hook or custom tool to study here - those belong in your **Line agent code** on
Cartesia's platform. What the bridge forwards to it (caller
