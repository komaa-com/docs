  ...
  entries:
    msteams_bridge:
      config:
        ...
        calling_port: 9442               # the media WebSocket -> /msteams/calling
        # Teams chat does NOT ride this plugin: it is the gateway's Teams platform on
        # 127.0.0.1:3978 at /api/messages. messages_port below is the MANAGED-BOT chat
        # relay only, and most installs never set it. See /expose.
        messages_port: 9444              # managed-bot chat relay -> /msteams/messages
...
| `calling_port` / `path` | the media WebSocket StandIn dials (default `9442`, `/msteams/calling`, the same calling port as every other StandIn plugin). `port` is still accepted as the old name for `calling_port`. StandIn dials `/msteams/calling/{call_id}`; the bare path has no route and 404s by design |
| `messages_port` / `messages_path` | the **managed-bot** chat relay (default `9444`, `/msteams/messages`), used only when your identity runs on the StandIn Managed Bot. Ordinary Teams chat does not come here: it goes to the gateway's Teams platform on `127.0.0.1:3978` at `/api/messages`. See [Expose your agent](/expose) |
...
| Field in StandIn | Public path | Local target |
|---|---|---|
| **Agent calling URL** | `wss://<your-public-host>/msteams/calling` | `127.0.0.1:9442`, path `/msteams/calling` (`msteams-bridge serve`) |
| **Agent chat URL** | `https://<your-public-host>/api/messages` | `127.0.0.1:3978`, path `/api/messages` (the gateway's Teams platform) |
...
[Expose your agent](/expose) for the two commands and the probe recipes.
