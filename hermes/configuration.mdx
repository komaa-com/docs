...
<Note>
  **The secrets are the exception to the prefix.** They read `MSTEAMS_CALL_SECRET`,
  `MSTEAMS_CALL_CALLING_SECRET` and `MSTEAMS_CALL_MESSAGES_SECRET` - not `TEAMS_CALL_*`. Everything
  else on this page keeps the `TEAMS_CALL_*` prefix it has always had.
</Note>
  ...
  entries:
    msteams_call:
      config:
        ...
        secret: ${MSTEAMS_CALL_SECRET}   # must equal the secret StandIn shows you
...
```bash
MSTEAMS_CALL_SECRET=...              # the ONE connection secret - covers calling AND messages
AZURE_FOUNDRY_API_KEY=...            # realtime key (also used by the gateway)
# fully env-only is fine too:
TEAMS_CALL_HOST=0.0.0.0
...
| Key | Meaning |
|---|---|
| `secret` | **The connection secret.** One value covers BOTH lanes; StandIn issues it per connection and you paste it once. Env: `MSTEAMS_CALL_SECRET`. Keep it in `.env` |
| `calling_secret` / `messages_secret` | Per-lane OVERRIDES, for deployments that insist on separate keys. When set they win over `secret`. Env: `MSTEAMS_CALL_CALLING_SECRET` / `MSTEAMS_CALL_MESSAGES_SECRET`. Most installs need neither |
...
|---|---|---|
| **Agent calling URL** | `wss://<your-public-host>:8443/msteams/calling` | `msteams-call serve` |
