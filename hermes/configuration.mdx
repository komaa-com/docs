  ...
  entries:
    msteams_bridge:
      config:
        ...
        messages_port: 8444              # managed-bot chat relay -> /msteams/messages
...
| `calling_port` / `path` | the media WebSocket StandIn dials (default `8443`, `/msteams/calling`). `port` is still accepted as the old name for `calling_port`. StandIn dials `/msteams/calling/{call_id}`; the bare path has no route and 404s by design |
| `messages_port` / `messages_path` | the **managed-bot** chat relay (default `8444`, `/msteams/messages`), used only when your identity runs on the StandIn Managed Bot. Ordinary Teams chat does not come here: it goes to the gateway's Teams platform on `127.0.0.1:3978` at `/api/messages`. See [Expose your agent](/expose) |
...
|---|---|---|
| **Agent calling URL** | `wss://<your-public-host>/msteams/calling` | `127.0.0.1:8443`, path `/msteams/calling` (`msteams-bridge serve`) |
