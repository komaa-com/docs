...
StandIn connects to your plugin **from the internet**, so both ports must be reachable - a public
host, or a tunnel. With Tailscale Funnel, expose them on two of its three slots:

```bash
tailscale funnel --bg --https=8443 8443    # calling  -> /msteams/calling
tailscale funnel --bg --https=10000 8444   # messages -> /msteams/messages
```

That gives you the two values StandIn asks for:

| Field in StandIn | Value |
|---|---|
| **Agent calling URL** | `wss://<machine>.<tailnet>.ts.net:8443/msteams/calling` |
| **Agent messages URL** | `https://<machine>.<tailnet>.ts.net:10000/msteams/messages` |
...
port in each - a bare host means the 443 slot, which is neither of these.

<Note>
  Funnel serves exactly **3** external ports: 443, 8443 and 10000. The layout above spends two of
  them, which leaves 443 free.

  Also running the **OpenClaw** plugin on the same machine? Three slots cannot cover two full runtimes
  (that would need four). Put OpenClaw on 443, keep Hermes calling on 8443 and Hermes messages on
  10000, or give each runtime its own tailnet machine. Register each runtime as its own connection with
  its own secret.
</Note>
