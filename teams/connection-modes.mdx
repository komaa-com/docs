...
|---|---|---|
| Calling (media WebSocket), both modes | `wss://host/msteams/calling` | `127.0.0.1:9442` on OpenClaw, `127.0.0.1:8443` on Hermes, path `/msteams/calling` |
| Chat, **bring your own Azure bot** | `https://host/api/messages` | `127.0.0.1:3978`, path `/api/messages`, on both runtimes |
| Chat, **StandIn Managed Bot** with the plugin's managed chat lane | `https://host/msteams/messages` | `127.0.0.1:9444` on OpenClaw, `127.0.0.1:8444` on Hermes, path `/msteams/messages` |
