...
You get **two secrets**, and they are not interchangeable:

| Secret | Signs | Config key |
|---|---|---|
| Calling | the media WebSocket handshake | `calling_secret` (Hermes) / `sharedSecret` (OpenClaw) |
| Messages | the chat relay HTTP body | `messages_secret` (Hermes) / `messagesSecret` (OpenClaw) |

They are separate so a key leaked from one surface cannot forge the other, and so they can be rotated
independently.
