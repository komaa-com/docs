...
<Note>
  ...
  On both OpenClaw and Hermes that public `/api/messages` must reach **`127.0.0.1:3978`, path
  `/api/messages`** - your runtime's own Teams channel, a different process and port from the voice
  plugin, which serves `/msteams/calling` on `9442` (OpenClaw) or `8443` (Hermes). See
  [Expose your agent](/expose) for the two mount commands.
</Note>
