...
1. In your [StandIn dashboard](https://standin.komaa.com/dashboard), set the identity's
   **Agent voice host** to the host (and port) from step 3 - StandIn appends `/voice/msteams/stream`.
...
To also show the avatar's face on the Teams tile (not just hear it), start the bridge with
`LIVEKIT_TILE_VIDEO=auto` - the bridge relays the agent's avatar video onto the caller's tile. Left
off (the default), the tile shows StandIn's built-in animated avatar. See
...
- **Bot joins the call but stays silent** - the Agent voice host is unreachable from the internet;
  re-check the tunnel.
