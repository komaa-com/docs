...
keywords: ["livekit-msteams-bridge example","LiveKit voice agent Teams tutorial","bitHuman avatar agent","agent dispatch agentName","uv run worker.py"]
---

The repo ships example projects so you can see a full working setup - an agent **and** the
...
<CardGroup cols={3}>
  <Card title="examples/voice-agent" icon="microphone" href="https://github.com/komaa-com/livekit-msteams-bridge/tree/main/examples/voice-agent">
    A minimal LiveKit voice agent (OpenAI STT/LLM/TTS + silero VAD). `worker.py`, registers as
    `standin-voice-agent`.
  </Card>
  <Card title="examples/video-agent" icon="robot" href="https://github.com/komaa-com/livekit-msteams-bridge/tree/main/examples/video-agent">
    The same pipeline plus a lip-synced **bitHuman** avatar. `worker.py`, registers as
...
```bash
cd livekit-msteams-bridge/examples/voice-agent
```
...
(Plain pip works too: `pip install -r requirements.txt && python worker.py dev`. Use `start`
...
<Warning>
  ...
  `standin-voice-agent` and `standin-avatar-agent`). A mismatch is the classic silent failure: the room
  is created, the caller hears nothing, and the worker never gets a job.
</Warning>

The bridge listens on `ws://<host>:8080/msteams/calling/{callId}` (`PORT` and `WS_PATH` move the
...
<Tabs>
  <Tab title="Tailscale Funnel">
    ...
    The proxy target must repeat the path. Mapping a bare port
    (`tailscale funnel --bg --https=8443 8080`) also works and gives you
    `wss://<machine>.<tailnet>.ts.net:8443/msteams/calling`, but it spends one of only three
    external ports.
  </Tab>
...
1. In your [StandIn dashboard](https://standin.komaa.com/dashboard), set the identity's
   **Agent calling URL** to the `wss://` URL from step 3. Leave the **Agent messages URL** empty.
...
Run it the same way - from `examples/video-agent`, `uv run worker.py dev` - and restart the bridge with
...
[Configuration -> Avatar video](/livekit/configuration#avatar-video).
...
Each agent folder ships a `Dockerfile`; pass the environment at runtime rather than baking it in. From
...
```bash
docker run --env-file .env standin-voice-agent
```

The avatar (`examples/video-agent`) additionally needs the model mounted:
...
- **WebSocket rejected with `401`** - `BRIDGE_SECRET` does not match the secret in StandIn, or the
  registered URL is missing the `/msteams/calling` path.
...
- **Bot joins the call but stays silent** - the Agent calling URL is unreachable from the internet;
  re-check the tunnel. Probing it with curl needs `--http1.1`, and a plain `GET` returns `404` even
  when the mount is fine.
