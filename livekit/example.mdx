...
The bridge listens on `ws://<host>:9442/msteams/calling/{callId}` (`PORT` and `WS_PATH` move the
...
<Tabs>
  <Tab title="Tailscale Funnel">
    ...
    `/msteams/calling` on port `9442`, and no `/api/messages` because this bridge is voice only.
...
<Tip>
  Verify locally with `curl http://127.0.0.1:9442/healthz` (returns `ok`). A plain `GET` to
