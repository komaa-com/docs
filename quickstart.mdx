  ...
  <Tab title="OpenClaw (npm)">
    <Steps>
      ...
      <Step title="Configure">
        Add a `msteams-call` entry to your OpenClaw config - `sharedSecret` must match the value you set in StandIn:
        ```jsonc
        {
          "plugins": {
            "entries": {
  ...
  <Tab title="Hermes (pip)">
    <Steps>
      <Step title="Install">
        ...
        progress panel. Enablement happens in the config below: `msteams_call`
        under `plugins.enabled` is all Hermes needs.
      </Step>
      <Step title="Configure">
        In `~/.hermes/config.yaml` under `plugins.entries.msteams_call.config` (secrets in `.env`):
        ```yaml
        plugins:
          enabled: [msteams_call]
          entries:
            msteams_call:
              config:
                secret: ${MSTEAMS_CALL_SECRET}   # must match the secret set in StandIn
                host: 127.0.0.1
                port: 8443
...
<Warning>
  ...
  **8443** (Hermes); path `/msteams/calling`. A `sharedSecret` mismatch fails the HMAC handshake
