  ...
  <Tab title="Hermes (pip)">
    <Steps>
      ...
      <Step title="Run + call">
        ...
        ```
        In your StandIn dashboard, register your **Agent calling URL** (`wss://<your-public-host>/msteams/calling` via a path-mapped funnel; local bind `ws://<host>:8443/...`)
...
<Warning>
  **StandIn connects to your plugin from the internet**, so both lanes must be reachable - expose them
  ...
  front of **9442** (OpenClaw) or **8443** (Hermes); chat is `/api/messages` in front of **3978** on
