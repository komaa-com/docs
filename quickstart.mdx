...
To review the script before running it, download it first, check its SHA-256, then run it:
...
<Tabs>
  <Tab title="macOS / Linux">
    ...
    Check the SHA-256 (must print `install.sh: OK`; on macOS use `shasum -a 256 -c -`):

    ```bash
    echo "8acb0c3a631dd957576560ec3e95cef9471de190ced56feac54f5524fcf0f6ea  install.sh" | sha256sum -c -
    ```
  ...
  <Tab title="Windows (PowerShell)">
    ...
    Check the SHA-256 (must print `True`):

    ```powershell
    (Get-FileHash install.ps1).Hash -eq "F412EE854AFEA1A01FD72D0433BF22B45DAEECF0DC7FA512012CA5ECDF9A2CA9"
    ```
...
Checksums are for the current release and change when the installer is updated; always fetch over
HTTPS from `standin.komaa.com`.
